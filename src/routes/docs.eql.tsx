...
export const Route = createFileRoute("/docs/eql")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "EQL grammar — seek, fetch, scan, place, erase — plus filters, projections, and bindings." },
...
function Page() {
  return (
    <DocsShell eyebrow="Reference" title="EQL" toc={[
      ...
      <p className="text-[18px] text-ink">
        EQL is a small expression-oriented query language with a lexer,
        recursive-descent parser, AST, and executor. It produces the same{" "}
        <code>Filter</code> and <code>QueryPlan</code> objects the SDK
        builders produce.
      </p>
...
{`seek in <vault>
    ...
    yield`}
      ...
      <ul>
        <li><code>top</code> defaults to 10 — unbounded when a <code>threshold</code> range is supplied.</li>
        <li><code>rank_by &lt;field&gt;</code> re-sorts results by a metadata field; default is distance.</li>
        <li><code>project</code> trims the returned payload.</li>
      </ul>
...
value      := "string" | number | true | false`}
      ...
      <p>
        Numbers without a decimal point are integers. <code>contains</code>{" "}
        is a substring match on string fields. Comments start with{" "}
        <code>#</code> and run to end of line.
      </p>
