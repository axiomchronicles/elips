...
export const Route = createFileRoute("/docs/internals/transaction-engine")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "How ELIPS buffers and commits atomic write batches — eager validation, RAII auto-rollback, and the single-writer isolation model." },
      { property: "og:title", content: "Transaction engine — ELIPS" },
      { property: "og:description", content: "Transaction / TransactionVault, eager validation, commit and rollback, and the Python context-manager binding." },
...
function Page() {
  return (
    <DocsShell eyebrow="Internals" title="Transaction engine" toc={[
      ...
      <p className="text-[18px] text-ink">
        ELIPS transactions are atomic write batches under the
        single-writer model. <code>Transaction</code> buffers{" "}
        <code>place</code> and <code>erase</code> calls; commit applies
        them in order, each one WAL-appended before the in-memory
        mutation. The destructor auto-rolls back if neither{" "}
        <code>commit()</code> nor <code>rollback()</code> ran.
      </p>
...
{`struct PendingOp {
    ...
    std::optional<RecordID>    id;                // pre-generated or explicit
      ...
      <p>
        Inserts capture vector + payload + pre-generated id; erases
        capture only the target id. The buffer is{" "}
        <code>std::vector&lt;PendingOp&gt;</code> — operations apply in
...
{`void Transaction::enqueue_place(std::string vault, const Vector& vector,
    ...
    ops_.push_back(PendingOp{false, std::move(vault), vector,
      ...
      <p>
        Validating up front means <code>commit()</code> can never fail
        mid-batch on dimension or finiteness grounds — the only failure
        modes left are storage IO errors. Without this, atomicity would
        be impossible to promise: half the batch could end up applied
        before a bad vector tripped validation.
      </p>
...
{`RecordID TransactionVault::place(const Vector& vector, Payload payload,
    ...
    return assigned;          // returned before commit
      ...
      <p>
        Generating the UUIDv7 at enqueue time lets callers log or
        cross-reference the id during the transaction, even though the
        record is not yet visible to readers.
      </p>
...
{`void Transaction::commit() {
    ...
    done_ = true;
      ...
      <p>
        Each operation is WAL-appended individually before the in-memory
        mutation. A crash mid-commit is consistent: WAL replay reapplies
        every operation that made it to the log; everything past the
        crash is lost cleanly. Under the single-writer lock no other
        mutator can interleave.
      </p>
...
Transaction::~Transaction() {
    if (!done_) {
      ...
      <p>
        Rollback is trivial — nothing was applied. The destructor calls
        it on any non-finalised transaction, so exceptions and early
        returns never leave a half-built batch lying around.
      </p>
...
{`with db.begin_transaction() as txn:
    ...
    # raised exc  → __exit__ does NOT commit; destructor rolls back`}
      ...
      <p>
        The binding uses a <code>TransactionHolder</code> that keeps the
        Python <code>Database</code> reference alive for the
        transaction's lifetime, so the C++ <code>ElipsInstance</code>{" "}
        outlives the <code>Transaction</code> as required.
      </p>

      <h2 id="isolation">Isolation level</h2>
      <p>
        Atomic batched writes under a single-writer lock — effectively{" "}
        <strong>serializable</strong> isolation. All transactions are
        serialised by the database lock. Reads within a transaction do{" "}
        <em>not</em> see uncommitted writes; <code>seek</code> always
        reads from the committed state.
      </p>
      <ul>
        <li><strong>Atomicity</strong> — all or none, via rollback. WAL provides durability.</li>
        <li><strong>Consistency</strong> — eager validation rejects bad inputs before they enter the buffer.</li>
        <li><strong>Isolation</strong> — single writer prevents dirty reads and lost updates.</li>
        <li><strong>Durability</strong> — every operation hits the WAL during commit, flushed per <code>Durability</code>.</li>
      </ul>
