...
export const Route = createFileRoute("/docs/cli")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "The elips command — info, vaults, stats, verify, query, checkpoint, import, export, bench." },
  ...
  return (
    <DocsShell eyebrow="Reference" title="CLI">
      <p className="text-[18px] text-ink">
        <code>elips</code> is a local command-line tool for inspection,
        maintenance, and EQL execution. No server. The build produces an{" "}
        <code>elips</code> binary you can drop on your <code>PATH</code>.
      </p>

      <h2>Shape</h2>
      <CodeBlock lang="bash">{`elips <command> <db_path> [options]`}</CodeBlock>
      <p>
        When creating a new database, pass <code>--dimension</code> (and
        optionally <code>--metric</code>, <code>--index</code>). Existing
        databases read their identity from disk.
      </p>
...
elips query /my_db --file query.eql`}
      ...
      <h2>Import / Export</h2>
      <p>JSON Lines, one record per line: <code>{`{"id","vector","data"}`}</code>.</p>
      <CodeBlock lang="bash">
...
elips import /my_db --vault docs --input docs.jsonl --dimension 3`}
      ...
      <h2>Output</h2>
      <p><code>query</code> prints one JSON object per result row:</p>
