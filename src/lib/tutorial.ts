...
const beginner: Lesson[] = [
  {
    ...
    blocks: [
      { kind: "lede", text: "ELIPS lives inside your process. It is to vector search what SQLite is to relational data." },
      { kind: "p", text: "From docs/overview/introduction.md: ELIPS is designed for CLIs, desktop apps, notebooks, edge workloads, single-process APIs, and tests — anything that wants local, in-process retrieval without operating a separate service." },
      { kind: "diagram", name: "WhyVectorDB", caption: "Why a vector DB exists: text → embedding → nearest-neighbor lookup over a billion-row corpus in milliseconds." },
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: "p", text: "The wheel ships the C++23 core as a compiled pybind11 extension (see docs/python/bindings/pybind11-architecture.md)." },
      { kind: "h", text: "C++" },
      { kind: "code", lang: "bash", code: `git clone https://github.com/axiomchronicles/ellips.git
...
cmake --build build -j` },
      ...
      { kind: "h", text: "Verify" },
      { kind: "code", lang: "python", code: `import elips
db = elips.open(":memory:", dimension=4, metric="cosine")
print(db.vaults())   # ["default"]` },
      { kind: "note", text: `":memory:" databases never touch the disk. Perfect for tests and tutorials.` },
    ],
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "An ELIPS database is a directory. Same way a SQLite database is a file." },
      { kind: "diagram", name: "OnDiskLayout", caption: "What ELIPS writes on disk. Each file is independently meaningful and atomically replaced." },
      { kind: "code", lang: "python", filename: "first_db.py", code: `import elips
...
print(hit.id, hit.distance, hit.payload)` },
      { kind: "p", text: "Kill the process and run it again. The record is still there — ELIPS replayed the WAL on open." },
    ],
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "A Record is id + vector + optional payload + optional document attachment + optional lineage." },
      { kind: "diagram", name: "ObjectModel", caption: "Instance → vaults → records. Records are the only objects the index sees." },
      { kind: "code", lang: "python", code: `notes.place(
    vector=[0.1] * 128,
    payload={"author": "ann", "kind": "design"},
      ...
      { kind: "note", text: "Payloads are read back with every hit. Don't put megabytes there; that's what document attachments are for." },
    ],
  },
  {
    ...
    blocks: [
      ...
      { kind: "diagram", name: "Persistence", caption: "place / erase append to the WAL first, then mutate the in-memory store." },
      { kind: "code", lang: "python", code: `rid = notes.place([0.1, 0.2, 0.3, 0.4], {"tag": "a"})
notes.erase(rid)   # marks tombstone; checkpoint/compact reclaims space` },
      { kind: "p", text: "From docs/storage.md: the WAL supports insert, erase, and insert_ex (for records carrying DocumentAttachment, ChunkInfo, or EmbeddingLineage). Crash recovery restores full record state, not only vectors." },
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: "diagram", name: "QueryPath", caption: "App → vault → planner → metadata → index → ranked hits." },
      { kind: "code", lang: "python", code: `hits = notes.seek(
...
for h in hits:
    print(h.id, h.distance, h.payload)` },
      { kind: "p", text: "From docs/cpp/api-reference/vault.md: SearchResult carries id, distance, data, document, chunk, lineage — all hydrated from the authoritative record store." },
    ],
  },
  {
    slug: "07-filters",
    ...
    blocks: [
      { kind: "lede", text: "If the filter is selective, the planner short-circuits through MetadataIndex before ever touching the vector index." },
      { kind: "code", lang: "python", code: `hits = docs.seek(
    query=embed("alpha"),
    top=10,
    filter={
        "kind": "design",
  ...
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "place_document() embeds, chunks, and stores in one call — and records exactly which model produced each vector." },
      { kind: "code", lang: "python", code: `docs = db.vault("documents")
docs.place_document(
...
for hit in docs.seek_text("alpha", top=3):
    ...
    print(hit.lineage.provider, hit.lineage.model, hit.lineage.revision)` },
      { kind: "p", text: "Lineage is what lets you re-embed only the records produced by a stale model. It's also what powers exact citations." },
    ],
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "Pure ANN misses exact keywords. Pure lexical misses paraphrases. Fuse both." },
      { kind: "code", lang: "python", code: `hits = docs.seek_hybrid(
    query=embed("alpha design note"),
    text="alpha design note",
    top=5,
    lexical_weight=0.25,   # default per docs/cpp/api-reference/vault.md
)` },
      { kind: "p", text: "lexical_weight ∈ [0, 1]. 0 means pure vector; 1 means pure lexical. The fused score is on each SearchResult." },
    ],
  },
  {
    slug: "10-cli-tour",
    ...
    blocks: [
      { kind: "lede", text: "The elips binary is your maintenance toolbox. Built from src/ alongside the library." },
      { kind: "code", lang: "bash", code: `elips info /my_db
elips vaults /my_db
elips stats /my_db
...
elips bench /tmp/bench_db --count 100000 --dim 768` },
      { kind: "p", text: "Output of elips query is one JSON object per result row — pipe it through jq, store it in files, diff between runs." },
    ],
  },
];
...
const intermediate: Lesson[] = [
  {
    ...
    blocks: [
      { kind: "lede", text: "Reach for EQL when filter dicts stop being expressive enough. It compiles to the same QueryPlan the SDKs use." },
      { kind: "h", text: "Statement grammar" },
      { kind: "code", lang: "text", code: `seek in <vault>
...
erase from <vault> id "<uuid>"` },
      ...
      { kind: "code", lang: "text", code: `filter   := term (("and" | "or") term)* | "not" filter | "(" filter ")"
term     := field <comparator> value
          | field "in" [ value, ... ]
          | field "contains" "substring"
comparator := = | != | < | <= | > | >=` },
      { kind: "h", text: "Run from Python" },
      { kind: "code", lang: "python", code: `db.query("""
  seek in articles
    nearest $embedding
    top 10
...
""", bindings={"embedding": embed(query)})` },
      ...
      { kind: "code", lang: "bash", code: `elips query /my_db --eql 'seek in docs nearest [0.1,0.2,0.3] top 5 yield'
elips query /my_db --file ranking.eql` },
  ...
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "ELIPS validates every operation as you enqueue it, so commit() is a pure write." },
      { kind: "diagram", name: "TransactionLifecycle", caption: "Three terminal paths: commit, rollback, or destructor-rollback. done_ flips once." },
      { kind: "p", text: "From docs/cpp/api-reference/transaction.md: TransactionVault::place() validates dimension and finiteness at enqueue, throwing DimensionMismatch or InvalidVector. If enqueue succeeds for every op, commit() cannot fail validation." },
      { kind: "code", lang: "python", code: `with db.begin_transaction() as txn:
    v = txn.vault("notes")
    v.place(vector1, {"tag": "a"})
    v.place(vector2, {"tag": "b"})
    # clean exit -> commit
    # exception  -> rollback (destructor)` },
      { kind: "code", lang: "cpp", code: `auto txn = db->begin_transaction();
auto& tv = txn.vault("notes");
tv.place(vector1, {{"tag", std::string{"a"}}});
tv.place(vector2, {{"tag", std::string{"b"}}});
txn.commit();   // or txn.rollback();` },
      { kind: "note", text: "Unlike Vault::place(), TransactionVault::place() does not normalize the vector at enqueue — normalization happens inside the per-op place() during commit." },
    ],
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "IDENTITY is the durable source of truth. Reopening with a conflicting dimension is a typed error, not a footgun." },
      { kind: "h", text: "The four core enums (docs/cpp/api-reference/config.md)" },
      { kind: "code", lang: "cpp", code: `enum class Metric      { cosine, euclidean, dot_product };
...
enum class AccessMode  { read_write, read_only };` },
      ...
      { kind: "code", lang: "cpp", code: `elips::Config{}
    .dimension(768)
  ...
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "Open any ELIPS directory and you can tell what state it's in without running a single command." },
      { kind: "diagram", name: "OnDiskLayout", caption: "Snapshot mode vs. segmented mode share LOCK, IDENTITY, wal.log, and the optional TEXT_EMBEDDER manifest." },
      { kind: "code", lang: "text", code: `/my_db/
...
└── elips.snapshot                    # snapshot mode (older / opt-in)` },
      { kind: "p", text: "From docs/storage.md: segmented mode is the default. Snapshot mode is preserved for compatibility and atomic rename (elips.snapshot.tmp → elips.snapshot)." },
    ],
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "These are the two maintenance verbs. Schedule them; don't pray for them." },
      { kind: "ul", items: [
      ...
      { kind: "p", text: "Segmented mode writes one fresh segment per vault, rewrites elips.manifest, and removes obsolete segment files. Snapshot mode writes elips.snapshot.tmp, atomic-renames to elips.snapshot, removes segmented artifacts." },
      { kind: "code", lang: "python", code: `db.checkpoint()   # flush + truncate WAL
db.compact()      # rebuild indexes + checkpoint` },
      { kind: "note", text: "Run checkpoint() from a job runner on a cadence that matches your durability target. Run compact() after large delete bursts." },
    ],
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "Open the same database as read_only from any number of processes. They take a shared flock; mutations are rejected at the API boundary." },
      { kind: "code", lang: "python", code: `replica = elips.open("/var/data/elips", read_only=True)
for hit in replica.vault("docs").seek(q, top=10):
    ...` },
      ...
      { kind: "p", text: "This is the supported mode for shared-reader analytics and read-fanout serving. Writers run elsewhere; replicas pick up new segments on the next open or rotation." },
    ],
  },
  {
    slug: "17-embedders",
    ...
    blocks: [
      { kind: "lede", text: "An embedder is whatever implements TextEmbedderPort. ELIPS records its identity in TEXT_EMBEDDER.manifest so reopens are deterministic." },
      { kind: "h", text: "Use the built-in local embedder" },
      { kind: "code", lang: "python", code: `db = elips.open("/data/db", dimension=128, auto_text_embedder=True)` },
      { kind: "h", text: "Custom embedder" },
      { kind: "code", lang: "python", code: `class MyEmbedder(elips.TextEmbedder):
    dimension = 768
    name = "my-embedder/v1"
    revision = "2026-06"
...
db = elips.open("/data/db", dimension=768, embedder=MyEmbedder())` },
      { kind: "p", text: "From docs/cpp/api-reference/config.md: TextEmbedderPort exposes provider_name, model_name, revision_name, backend_name, output_dimension. ELIPS uses these to fingerprint the manifest." },
      { kind: "note", text: "Non-rehydratable external embedders store metadata only. Reopening without the same embedder fails seek_text() with an actionable ConfigError." },
    ],
  },
  {
    ...
    blocks: [
      ...
      { kind: "code", lang: "cpp", code: `auto plan = vault.explain_seek(query, /*top=*/10, filter);
...
// plan.index_type_name` },
      ...
      { kind: "p", text: "Treat strategy + candidate_count as the two telemetry numbers you actually emit per query." },
    ],
  },
  {
    slug: "19-error-model",
    ...
    blocks: [
      ...
      { kind: "code", lang: "text", code: `std::runtime_error
└── elips::ElipsError
  ...
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "Python doesn't reimplement anything. It binds the same ElipsInstance and Vault that C++ users hold." },
      { kind: "p", text: "From docs/python/bindings/pybind11-architecture.md: there is a low-level surface (elips.open, Database, Vault — direct binding mirrors) and a modern surface (connect(), Engine, Arena — opinionated Pythonic wrappers)." },
      { kind: "code", lang: "python", code: `# low-level — mirrors C++ shapes
...
const advanced: Lesson[] = [
  {
    ...
    blocks: [
      { kind: "lede", text: "Every ELIPS feature lives on exactly one of five layers. Knowing which one is the difference between a one-line patch and a refactor." },
      { kind: "diagram", name: "SystemShape", caption: "SDKs → ElipsInstance → Vault → planner + ports (index/metadata/text/GPU) → storage (WAL + segments)." },
      { kind: "p", text: "From docs/architecture/system-overview.md: each layer talks down to the next via abstract ports. Nothing in the planner depends on a concrete index. Nothing in the index depends on a concrete storage layout." },
      { kind: "diagram", name: "AlgorithmPorts", caption: "Ports & adapters: the planner sees IndexPort, MetadataIndex, TextEmbedderPort, GpuPort — not concrete classes." },
    ],
  },
  {
    ...
    blocks: [
      ...
      { kind: "p", text: "Every frame is length-prefixed and hashed. Recovery replays the longest valid prefix and truncates a corrupt tail without aborting." },
      { kind: "h", text: "Segments" },
      { kind: "p", text: "checkpoint() writes one segment per vault: vault_<n>_<epoch>.segment. The new elips.manifest references the new epoch. Old segment files are unlinked only after the manifest has been atomically replaced." },
      { kind: "h", text: "Snapshot mode (legacy / opt-in)" },
      { kind: "p", text: "Writes to elips.snapshot.tmp, fsync, rename to elips.snapshot. Single-file, atomic, simpler — slower for large databases." },
    ],
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "open() is the most important code path in any storage engine. ELIPS' is six steps long." },
      { kind: "diagram", name: "RecoveryFlow", caption: "lock → identity → embedder manifest → segments-or-snapshot → WAL replay → attach live WAL (skipped for read-only)." },
      { kind: "ul", items: [
      ...
      { kind: "p", text: "Corrupt WAL tails are tolerated by design — the database is the longest valid prefix that hashed cleanly." },
    ],
  },
  {
    slug: "24-hnsw",
    ...
    oneLiner: "Multi-layer skip-list-shaped proximity graph. Log-time approximate nearest neighbors.",
    blocks: [
      { kind: "lede", text: "ANN is mostly graph traversal in disguise. HNSW makes the disguise efficient." },
      { kind: "diagram", name: "HnswLayers", caption: "Top layer: long-range hubs. Bottom layer: every point. Search descends one layer at a time." },
      ...
      { kind: "p", text: "Increase ef_search to trade latency for recall at query time without rebuilding. Increase ef_construction or max_connections only at build time — and only if your recall plateau is below target." },
    ],
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "Concurrency control is in-process. ELIPS does not coordinate across processes for writes — that is what the file lock is for." },
      { kind: "p", text: "From docs/internals/lock-manager.md: locks are acquired in a fixed hierarchical order (database → vault → record). Acquiring out of order is a programmer error and is asserted in debug builds." },
      { kind: "ul", items: [
  ...
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "The transaction is the simplest object in the engine. That is by design." },
      { kind: "diagram", name: "TransactionLifecycle", caption: "Begin → enqueue (validated) → commit OR rollback OR destructor-rollback. done_ flips exactly once." },
      ...
      { kind: "code", lang: "cpp", code: `class Transaction {
public:
...
private:
  ...
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "EQL is not a toy. It is a real, four-stage compiler embedded in the engine." },
      { kind: "ul", items: [
      ...
      { kind: "p", text: "Bindings ($name) are typed at runtime — the executor checks that a $vector binding is the right dimension before the plan ever reaches a Vault." },
    ],
  },
  {
    slug: "28-gpu-engine",
    ...
    blocks: [
      { kind: "lede", text: "GPU support is optional, compile-time-selectable, and entirely behind a port. Nothing in the planner mentions CUDA." },
      { kind: "diagram", name: "GpuEngine", caption: "GpuPort + GpuMemoryPort + GpuKernelPort + GpuStreamPort + GpuIndexPort. Backends fulfill the contract." },
      { kind: "h", text: "The 13-method GpuPort (include/elips/gpu_engine/GpuPort.hpp)" },
      { kind: "code", lang: "cpp", code: `class GpuPort {
public:
    ...
    [[nodiscard]] virtual bool is_idle() const noexcept = 0;
      ...
      { kind: "p", text: "All operations return std::expected<T, GpuError>. The error enum: DeviceNotFound, InsufficientMemory, KernelLaunchFailed, TransferFailed, IndexBuildFailed, UnsupportedMetric, InitializationFailed, BackendUnavailable." },
    ],
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "GpuDeviceManager probes whatever was compiled in. GpuSelector picks the best device that matches policy." },
      { kind: "p", text: "Probe order (docs/internals/gpu-engine.md): Metal (Apple), CUDA, HIP, SYCL, Vulkan. Each probe init+inspect+shutdowns the backend inside a try/catch. Devices are then sorted by CAGRA support, then by total device memory descending." },
      { kind: "ul", items: [
      ...
      { kind: "diagram", name: "GpuAccelGlance", caption: "Probe → sort → select → initialize. Falls back per GpuPolicy." },
    ],
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "GPUs hate small launches. The DynamicBatcher exists to make them stop being small." },
      { kind: "diagram", name: "DynamicBatcher", caption: "Concurrent queries wait inside a tiny window, then ride one kernel launch as a batch." },
      ...
      { kind: "p", text: "GpuIngestionPipeline streams record vectors onto the device. GpuQuantizationPipeline trains IVF-PQ centroids and pre-encodes residuals. GpuSearchPipeline orchestrates the batched distance + top-k path. GpuProfiler instruments every stage." },
      { kind: "code", lang: "cpp", code: `auto cfg = elips::Config{}
    .dimension(768)
    .metric(elips::Metric::cosine)
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: "p", text: "The elips bench command builds a synthetic database at a given dimension and count, then runs warm-cache queries — use it as a sanity floor when comparing hardware or builds." },
    ],
  },
  {
    ...
    blocks: [
      { kind: "lede", text: "ELIPS is embedded — production is your binary, on a server, with disks you trust." },
      { kind: "diagram", name: "ElipsAgenticSystem", caption: "What the layered stack looks like when an agent runtime owns its own retrieval." },
      { kind: "ul", items: [
      ...
      { kind: "p", text: "You've now seen every concept in the engine. Each one has its own page in /docs and its own chapter here. The roadmap (docs/roadmap.md) is what is coming next." },
    ],
  },
];
